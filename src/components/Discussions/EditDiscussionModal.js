...
const EditDiscussionModal = ({ isOpen, onClose, discussion }) => {
  ...
  // Populate form when discussion changes
  useEffect(() => {
    if (discussion) {
      setFormData({
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => discussionsAPI.updateDiscussion(id, data),
    onSuccess: (data) => {
    ...
    onError: (error) => {
      ...
      setErrors({ submit: 'Failed to update discussion. Please try again.' });
    }
  });
  ...
  const submitUpdate = () => {
    ...
    updateMutation.mutate({ id: discussion.id, data: discussionData });
  };
    ...
    >
      <form
        ...
        onTouchStart={(e) => {
          ...
          if (target.type === 'submit' && updateMutation.isPending) {
            e.preventDefault();
            e.stopPropagation();
          }
        }}
